...
#setup ----------------
{
  ...
  aktox=readRDS(paste0(out_dir,"outdf_akde_tox_corrected.rds"))
...
#distance -------------------
{
  dist<- readRDS(paste0(out_dir,"pig_weekly_distance_ctmm.rds"))
...
res_aer2 = recalculateResiduals(simout_aer, group = distaer$week)
...
res_trap2 = recalculateResiduals(simout_trap, group = disttrap$week)
...
res_tox2 = recalculateResiduals(simout_tox, group = disttox$week)
...
testSpatialAutocorrelation(aer_test_resid2,groupLocations$mX, groupLocations$mY)
...
groupLocations = aggregate(disttrap[,c("mX","mY")], list(disttrap$animalid), mean)
trap_test_resid2 = recalculateResiduals(trap_test_resid, group = disttrap$animalid, rotation="estimated")
testSpatialAutocorrelation(trap_test_resid2,groupLocations$mX, groupLocations$mY)
...
testSpatialAutocorrelation(tox_test_resid2,groupLocations$mX, groupLocations$mY)
#no spatial autocorrelation

### tox and trap temporal
...
trap_res2 = recalculateResiduals(trap_res, group = as.factor(disttrap$animalid))
plot(trap_res2)
...
trap_res_rps2 = recalculateResiduals(trap_res_rps, group = as.factor(disttrap$animalid))
plot(trap_res_rps2)
...
tox_res2 = recalculateResiduals(tox_res, group = as.factor(disttox$animalid))
plot(tox_res2)
...
tox_res_rps2 = recalculateResiduals(tox_res_rps, group = as.factor(disttox$animalid))
plot(tox_res_rps2)
...
#speed -------------------
{
  speed<- readRDS(paste0(out_dir,"pig_weekly_speed_ctmm.rds"))
...
res_aers2 = recalculateResiduals(simout_aers, group = speedaer$week)
...
res_traps2 = recalculateResiduals(simout_traps, group = speedtrap$week)
...
res_toxs2 = recalculateResiduals(simout_toxs, group = speedtox$week)
...
groupLocations = aggregate(speedaer[,c("mX","mY")], list(speedaer$animalid), mean)
aer_test_resids2 = recalculateResiduals(aer_test_resids, group = speedaer$animalid, rotation="estimated")
testSpatialAutocorrelation(aer_test_resids2,groupLocations$mX, groupLocations$mY)
# spatial autocorrelation
...
groupLocations = aggregate(speedtrap[,c("mX","mY")], list(speedtrap$animalid), mean)
trap_test_resids2 = recalculateResiduals(trap_test_resids, group = speedtrap$animalid, rotation="estimated")
testSpatialAutocorrelation(trap_test_resids2,groupLocations$mX, groupLocations$mY)
#no spatial autocorrelation
...
aer_res_sp_rp2 = recalculateResiduals(aer_res_sp_rp, group = as.factor(speedaer$animalid))
plot(aer_res_sp_rp2)
...
res_speed_rps_aer=sdmTMB(weekly_md_km_hr ~ (1|animalid)+
                           Removal.Type*removal.period.akdecalc*sex,
                         data=speedaer,
                         mesh=meshaer_sp,
                         time='week',
                         ...
                         spatiotemporal = "ar1",
                         family=Gamma(link='log'))
sanity(res_speed_rps_aer)

aer_res_sp_rps <- simulate(res_speed_rps_aer, nsim = 250, type = "mle-mvn") %>% 
  dharma_residuals(res_speed_rps_aer, return_DHARMa = TRUE)
aer_res_sp_rps2 = recalculateResiduals(aer_res_sp_rps, group = as.factor(speedaer$animalid))
plot(aer_res_sp_rps2)
plot(aer_res_sp_rps2$simulatedResponse)
DHARMa::testDispersion(aer_res_sp_rps2)
...
tox_res_rp2 = recalculateResiduals(tox_res_rp, group = as.factor(distaer$animalid))
plot(tox_res_rp2)
...
tox_res_rps2 = recalculateResiduals(tox_res_rps, group = as.factor(distaer$animalid))
plot(tox_res_rps2)
plot(tox_res_rps2$simulatedResponse)
DHARMa::testDispersion(tox_res_rps2)
